...
function startbody(){
	...
	for(var i = 0; i < USERHEAD.length; i++){
		var li = creat('li');
		li.innerHTML = USERHEAD[i].textEn;
...
function submit(){
	...
	boardMac.onchange = function(){
		...
		$.ajax({
			...
			success: function(data){
				console.log(data);
				if(data.obj != 0){
					alern('MACAddr'+data.obj.machName+'repeat!');
					that.value = "";
				}
			}
		})
	...
	bodySubmit.onclick = function(){
		...
		if(boardpPurview.name == ""){
			userError += 'Superior Group Is Null!</br>';
		}
		if(boardMachcode.value == ""){
			userError += 'Machine ID Is Null!</br>';
		}
		if(boardMachname.value == ""){
			userError += 'Machine Name Is Null!</br>';
		}
		if(boardModel.name == ""){
			userError += 'Machine Type Is Null!</br>';
		}
		if(boardMac.value == ""){
			userError += 'MAC Addr Is Null!</br>';
		}
		if(boardOperator.name == ""){
			userError += 'Operator Is Null!</br>';
		}
		if(boardConcur.value == ""){
			userError += 'Business Way Is Null!</br>';
		}
		if(boardBuys.style.display == 'table-row'&&boardBuy.value == ""){
			userError += 'Date Of Purchase Is Null!</br>';
		}
		if(boardRentstarts.style.display == 'table-row'&&boardRentstart.value == ""){
			userError += 'Lease Start Date Is Null!</br>';
		}
		if(boardRentends.style.display == 'table-row'&&boardRentend.value == ""){
			userError += 'Lease End Date Is Null!</br>';
		}
