...
function start(){
  ...
  LIST = [
    ...
    ['Sold','Refunded'],
    ['Select...','Completed','Undelivered'],
    ['Select...','Refunded By User','Unsuccess']
  ]
...
function selesForm(){
  ...
  submit.onclick = function(){
    ...
    var Start = startDate;
    var End = endDate;
    if(methods.value == 'Sold'){
      mehtodz = 0;
      if(unit.value == 'Select...'){
        unitz = -1;
      }else if(unit.value == 'Completed'){
        unitz = 1;
      }else if(unit.value == 'Undelivered'){
        unitz = 0;
      }else{
        alern('Status Cannot Null');
        return false;
      }
    }else if(methods.value == 'Refunded'){
      mehtodz = 1;
      if(units.value == 'Select...'){
        unitz = -1;
      }else if(units.value == 'Refunded By User'){
        unitz = 3;
      }else if(units.value == 'Unsuccess'){
        unitz = 2;
      }
    }else{
      alern('Transaction Type Cannot Null');
      return false;
    }
...
function tableRendering(allDate){
  ...
  if(type == 0){
    tableHead.innerHTML = '<tr><th>Machine Name</th><th>Machine ID</th><th>Commodity</th><th>Commodity ID</th><th>Order Number</th><th>Cargo Track</th><th>Pay way</th><th>Payment amount</th><th>User Info</th><th>Time</th><th>Status</th></tr>';
    if(allDate.length == 0){
    ...
    tableHead.innerHTML = '<tr><th>Machine Name</th><th>Machine ID</th><th>Commodity</th><th>Order Number</th><th>Cargo Track</th><th>Pay way</th><th>Payment amount</th><th>User Info</th><th>Time</th><th>Status</th><th>Operator</th><th>Mark</th></tr>';
    if(allDate.length == 0){
      table.innerHTML = '<tr><td colspan="11" style="color: #888888;">Null</td></tr>'
    }else{
      for(var i = 0; i < allDate.length; i++){
        ...
        td12.innerHTML = '<button onclick="alern(\''+allDate[i].refund_remark+'\',\'Mark\')">viewMark</button>';
        tr.appendChild(td1);
        tr.appendChild(td2);
        tr.appendChild(td3);
        tr.appendChild(td4);
  ...
  */
  var tableToExcel = (function() {
    ...
    return function(table, name) {
      ...
      link.download = 'Transaction Monitor' + names + '.xls';
      link.target = '_blank';
