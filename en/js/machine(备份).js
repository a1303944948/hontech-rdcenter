...
function startbody(){
	...
	for(var i = 0; i < MACHHEAD.length; i++){
		if(MACHHEAD[i].value == 1){
			...
			footList[i].style.visibility = 'visible';
			ulLi[i].style.borderTop = '2px #16b904 solid';
			ulLi[i].style.borderLeft = '1px #e5e5e5 solid';
			ulLi[i].style.borderRight = '1px #e5e5e5 solid';
			ulLi[i].style.marginTop = '-1px';
		...
		$.ajax({
			...
			success: function(data){
				...
				itemDivbbBtn.onclick = function(){
					...
					for(var i = 0;i < itemObject[0].length; i++){
						var tr = creat('tr');
						var tda = creat('td');
						var tdb = creat('td');
						var tdc = creat('td');
						var tdd = creat('td');
						tda.innerHTML = itemObject[0][i].recars;
						tda.title = itemObject[0][i].id;
						tdb.innerHTML = '<input class="user_body_right_foot_item_commoditya_tbody_inta" name="'+itemObject[0][i].recarsType+'" readonly="readonly" value="'+itemObject[0][i].goods+'" data-value="'+itemObject[0][i].goodsId+'" placeholder="请选择商品..."/>';
						tdc.innerHTML = '<input class="user_body_right_foot_item_commoditya_tbody_intb" value="'+itemObject[0][i].number+'" type="number"/>';
						var pride = itemObject[0][i].price;
						if(pride == ""){
							pride = 1;
						}
					...
					for(var o = 0; o < divList.length; o++){
						if(MACHROAD.type == 'a'){
							if(divList[o].style.backgroundColor == 'rgb(91, 192, 222)'){
								//特殊货道部分
								//初始数据渲染
								var machaStart = [];
								var machaObj = [];
								for(var i = 0; i < itemObject[o].length; i++){
									machaStart.push(itemObject[o][i].recars.split('-')[0]);
								}
								var hashStart=[];
								var hashObj = [];
								...
								for(var i = 0; i < hashStart.length; i++){
									...
									tdb.innerHTML = '<input class="user_body_right_foot_item_roada_table_int" value="'+hashObj[i]+'" type="number"/>';
									tdc.innerHTML = '<button class="user_body_right_foot_item_roada_table_clear"><img src="image/sc.png"/>delete</button>';
									tr.appendChild(tda);
									tr.appendChild(tdb);
									tr.appendChild(tdc);
									...
									for(var j = 0; j < roadaTableInt.length;j++){
										(function(q){
											...
											roadaTableClear[q].onclick = function(){
												roadaCount--;
												this.offsetParent.offsetParent.removeChild(this.offsetParent.parentNode);
								...
								roadaBtn.onclick = function(){
									if(roadaCount < 40){
										...
										var tdb = creat('td');
										var tdc = creat('td');
										tda.innerHTML = roadaCount;
										tdb.innerHTML = '<input class="user_body_right_foot_item_roada_table_int" type="number"/>';
										tdc.innerHTML = '<button class="user_body_right_foot_item_roada_table_clear"><img src="image/sc.png"/>delete</button>';
										tr.appendChild(tda);
										tr.appendChild(tdb);
										tr.appendChild(tdc);
						...
						if(MACHROAD.type == 'b'){
							if(divList[o].style.backgroundColor == 'rgb(91, 192, 222)'){
								...
								var machbObj = [];
								for(var i = 0; i < itemObject[o].length; i++){
									machbObj.push(itemObject[o][i].recarsType);
									machbStart.push(itemObject[o][i].recars.split('-')[0]);
								}
								var hashStart=[];
								var hashObj = [];
						...
						if(MACHROAD.type == 'c'){
							if(divList[o].style.backgroundColor == 'rgb(91, 192, 222)'){
								...
								var machcObj = [];
								for(var i = 0; i < itemObject[o].length; i++){
									machcObj.push(itemObject[o][i].recarsType);
									machcStart.push(itemObject[o][i].recars.split('-')[0]);
								}
								var hashStart=[];
								var hashObj = [];
		...
		success: function(data){
			...
			if(MACHROAD.type == 'a'){
				...
				for(var i = 0; i < hashStart.length; i++){
					...
					var tdd = creat('td');
					tda.innerHTML = hashStart[i];
					tdb.innerHTML = '<input class="user_body_right_foot_item_roada_table_selectType" value="'+hashType[i]+'" type="text"/>';
					var ul = creat('ul');
				...
				roadaBtn.onclick = function(){
					if(roadaCount < 40){
						...
						var tdd = creat('td');
						tda.innerHTML = roadaCount;
						tdb.innerHTML = '<input class="user_body_right_foot_item_roada_table_selectType" type="text"/>';
						var ul = creat('ul');
						ul.className = "user_body_right_foot_item_roada_table_selectUl";
						ul.style.left = "100px";
						ul.style.top = '24px';
						...
						roadaTableSelectType[roadaCount-1].value = roadaTableSelectUl[roadaCount-1].children[0].innerHTML;
						function roadaTableSelectClick(){
							for(var i = 0; i <　roadaTableSelectType.length; i++){
								(function(q){
									roadaTableSelectType[q].onfocus = function(){
